...
>
> Эта спецификация описывает **полную интеграцию с реальным QR API** (R3+). На проде сейчас реализован **плагинный каркас + mock-режим** для демо.
>
> | Возможность | MVP (R2, в проде) | Full (R3+, отложено) |
> |---|:---:|:---:|
> | Плагинная архитектура (`integrations/quickresto.js` + клиент-плагин) | ✅ | ✅ |
> | Mock-режим: генерация demo-чеков как `revenue_entry` | ✅ | ✅ |
> | Конфиг через UI: режим / credentials / venue / active | ✅ | ✅ |
> | Дедупликация по `externalId` | ✅ | ✅ |
> | Live-режим: реальные вызовы QR Back-Office API | ❌ stub возвращает 501 | ✅ |
> | Polling каждые 5 мин (авто-sync) | ❌ только manual sync | ✅ |
> | Webhook от QR (push при закрытии чека) | ❌ | ✅ |
> | Write-off engine (списания ингредиентов по рецептам) | ❌ только revenue | ✅ |
> | Маппинг QR menu_item → Orakul dish | ❌ | ✅ |
> | Bulk-import исторических чеков (90+ дней) | ❌ | ✅ |
> | Сторнирование revenue при cancel/refund в QR | ❌ | ✅ |
>
> **Та же scope-логика применяется к iiko-плагину** ([app/integrations/iiko.js](../../app/integrations/iiko.js)) — он повторяет тот же контракт.
